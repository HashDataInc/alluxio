...
/**
 * A stream for writing a file into TXCOS. The data will be persisted to a temporary directory on
 * the local disk and copied as a complete file when the {@link #close()} method is called.
 */
public class TXCOSOutputStream extends OutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(TXCOSOutputStream.class);

  /** The oss client for OSS operations. */
  private final COSClient mClient;

  /** The Bucket of TXCOS for object operations. */
  ...
  /**
   * Creates a name instance of {@link TXCOSOutputStream}.
   *
   * @param key the key of the file
   * @param bucket
   * @throws IOException if an I/O error occurs
   */
  public TXCOSOutputStream(String bucket, String key, COSClient client) throws IOException {
    Preconditions.checkArgument(key != null && !key.isEmpty(),
        "TXCOS path must not be null or empty.");
    Preconditions.checkArgument(bucket != null, "TXCOS bucket must not be null.");
   ...
   * uploaded to TXCOS. Once the file is uploaded, the temporary file is deleted.
   *
   * @throws IOException if an I/O error occurs
   */
  @Override
